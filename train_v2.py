...
def main():
    ...
    best_model_path = train_model(model, criterion_cls, criterion_seg, optimizer, train_loader, val_loader, num_epochs=30, patience=15, warmup_epochs=5)
    ...
    def evaluate(model, test_loader):
        ...
        with torch.no_grad():
            for images, input_ids, attention_masks, labels, ages, genders in test_loader:
                ...
                # Grad-CAM 可视化
                cam = grad_cam.generate(images[:1], torch.argmax(labels[0]).item(), device, is_gradcam_plus=True)
